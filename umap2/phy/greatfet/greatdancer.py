...
class GreatDancer(PhyInterface):
    """
    ...
    def __init__(self, app):
        super(GreatDancer, self).__init__(app, 'GreatFET')
    ...
    def connect(self, usb_device, max_ep0_packet_size=64):
        ...
        self.info("Connecting to host.")


    def disconnect(self):
        """ Disconnects the GreatDancer from its target host. """
        self.info("Disconnecting from host.")
    ...
    def send_on_endpoint(self, ep_num, data, blocking=False):
        ...
        """
        self.debug(f"EP{ep_num}/IN: <- {bytes(data)}")
    ...
    def _handle_transfer_complete_on_endpoint(self, endpoint_number, direction):
        ...
        if direction == self.HOST_TO_DEVICE:
            ...
            # callback.
                ...
                self.debug(f"EP{endpoint_number}/OUT -> {data}")
    ...
    def _bus_reset(self):
        ...
        self.debug("Host issued bus reset.")
